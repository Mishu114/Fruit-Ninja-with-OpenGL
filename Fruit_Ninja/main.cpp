...
int main()
{
    ...
    /*freopen("highScoreFile.txt", "w", stdout);
    freopen("highScoreFile.txt", "r", stdin);*/
    ...
    vector<float> tob{
-0.3650, 2.1600, 5.1000,
-0.3600, 2.0300, 5.1000,
-0.3800, 1.8750, 5.1000,
-0.5000, 1.7250, 5.1000,
-0.5850, 1.6200, 5.1000,
-0.7400, 1.5050, 5.1000,
-0.8400, 1.4200, 5.1000,
-0.8850, 1.1900, 5.1000,
-0.8350, 0.9350, 5.1000,
-0.7550, 0.7950, 5.1000,
-0.6450, 0.6950, 5.1000,
-0.5200, 0.5850, 5.1000,
-0.4200, 0.5500, 5.1000,
-0.2800, 0.4750, 5.1000,
-0.1200, 0.4200, 5.1000,
-0.0400, 0.4000, 5.1000,
    };
    ...
    string diffuseMapPath = "background.jpg";
    string specularMapPath = "background.jpg";
    ...
    string diffuseMaptub = "abc.jpg";
    string specularMaptub= "";
    unsigned int diffMaptub = loadTexture(diffuseMaptub.c_str(), GL_REPEAT, GL_REPEAT, GL_LINEAR_MIPMAP_LINEAR, GL_LINEAR);
    unsigned int specMaptub = loadTexture(specularMaptub.c_str(), GL_REPEAT, GL_REPEAT, GL_LINEAR_MIPMAP_LINEAR, GL_LINEAR);
    ...
    //fuller tob
    Curve  curve = Curve(tob, diffMaptub, specMaptub, 1);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //glm::mat4 identityMatrix = glm::mat4(1.0f); // make sure to initialize matrix to identity matrix first
        //glm::mat4 translateMatrix, rotateXMatrix, rotateYMatrix, rotateZMatrix, scaleMatrix, model;
        translateMatrix = glm::translate(identityMatrix, glm::vec3(-1.5, 0.0, 0.5));
        ...
        lightingShaderWithTexture.setMat4("model", model2);
        curve.draw(lightingShaderWithTexture, model2, glm::vec3(1.0f, 0.0f, 0.0f));
        ...
        if (score == 0)
        {
            ...
            string s;
            fstream my_file;
            my_file.open("high_score.txt", ios::in);
            if (!my_file) {
                cout << "No such file";
            }
            else {
            ...
            for (int i = s.size() - 1, mul = 1; i >= 0; i--, mul *= 10)
                high_score += (s[i] - '0') * mul;

            if (fruit_count > high_score) high_score = fruit_count;
            ...
            for (int i = 0; i < 3; i++)
                ss += (Arr[i] + '0');
            ofstream myfile("high_score.txt");
            myfile << ss << endl;
            myfile.close();
...
void func(unsigned int& cubeVAO, Shader& lightingShader, glm::mat4 alTogether)
{
    ...
    lightingShader.setMat4("model", model);
    drawCube(cubeVAO, lightingShader, model, 1.0f, 0.8, 0.5);
    ...
    lightingShader.setMat4("model", model);
    drawCube(cubeVAO, lightingShader, model, 234/255.0f, 236/255.0f, 236/255.0f);

    translateMatrix = glm::translate(identityMatrix, glm::vec3(gun_x-0.03f, gun_y+0.05, -1.51f));
...
void processInput(GLFWwindow* window)
{
    ...
    if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS) {
        gun_y += 0.002f + fruit_count * .00005;
    ...
    if (glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS) {
        gun_x += 0.002f + fruit_count * .00005;
    ...
    if (glfwGetKey(window, GLFW_KEY_K) == GLFW_PRESS) {
        if(score==0)
            score = 3, speed=.001, fruit_count=0;
        //count = 1;
        //num = 0;
    }
